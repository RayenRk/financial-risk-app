      ...
      <div className="p-8 space-y-6">
        ...
        <div>
          ...
          <p className="text-gray-400 mt-1">
            EPAM Systems — {quarters.length} quarters · {fmt(quarters[0]?.quarter_date)} to {fmt(latest?.quarter_date)}
          </p>
        </div>
        ...
        <SECTION title="Income Statement" subtitle="Revenue, gross profit, operating income, net income — $M">
          <ResponsiveContainer width="100%" height={260}>
            <AreaChart data={incomeData}>
              ...
              <Area type="monotone" dataKey="gross"     stroke="#8b5cf6" fill="url(#grossGrad)" strokeWidth={2} name="Gross Profit" />
              <Area type="monotone" dataKey="operating" stroke="#f59e0b" fill="url(#opGrad)"    strokeWidth={2} name="Operating Income" />
              <Area type="monotone" dataKey="net"       stroke="#22c55e" fill="url(#netGrad)"   strokeWidth={2} name="Net Income" />
            </AreaChart>
          </ResponsiveContainer>
        </SECTION>
        ...
        <SECTION title="Profit Margins" subtitle="Gross, operating, and net margin — percentage">
          <ResponsiveContainer width="100%" height={240}>
            <LineChart data={marginData}>
              ...
              <Legend wrapperStyle={{ color: '#9ca3af', fontSize: 12 }} />
              <Line type="monotone" dataKey="gross"     stroke="#8b5cf6" strokeWidth={2} dot={{ r: 4 }} name="Gross Margin" />
              <Line type="monotone" dataKey="operating" stroke="#f59e0b" strokeWidth={2} dot={{ r: 4 }} name="Operating Margin" />
              <Line type="monotone" dataKey="net"       stroke="#22c55e" strokeWidth={2} dot={{ r: 4 }} name="Net Margin" />
            </LineChart>
          </ResponsiveContainer>
        </SECTION>
        ...
        <SECTION title="Cash Flow & Debt" subtitle="Free cash flow, cash on hand, total debt — $M">
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={cashData}>
              ...
              <Tooltip {...TOOLTIP_STYLE} formatter={(v: number) => [`$${v}M`]} />
              <Legend wrapperStyle={{ color: '#9ca3af', fontSize: 12 }} />
              <Bar dataKey="fcf"  fill="#22c55e" radius={[4,4,0,0]} name="Free Cash Flow" />
              <Bar dataKey="cash" fill="#3b82f6" radius={[4,4,0,0]} name="Cash on Hand" />
              <Bar dataKey="debt" fill="#ef4444" radius={[4,4,0,0]} name="Total Debt" />
            </BarChart>
          </ResponsiveContainer>
        </SECTION>

        {/* Ratios chart */}
        <SECTION title="Key Ratios" subtitle="Current ratio, debt-to-equity, ROE, ROA">
            ...
            <div>
              ...
              <ResponsiveContainer width="100%" height={180}>
                <LineChart data={ratioData}>
                  ...
                  <Line type="monotone" dataKey="current" stroke="#3b82f6" strokeWidth={2} dot={{ r: 3 }} name="Current Ratio" />
                  <Line type="monotone" dataKey="de"      stroke="#f59e0b" strokeWidth={2} dot={{ r: 3 }} name="D/E Ratio" />
                </LineChart>
              </ResponsiveContainer>
            </div>
            ...
            <div>
              ...
              <ResponsiveContainer width="100%" height={180}>
                <LineChart data={ratioData}>
                  ...
                  <Tooltip {...TOOLTIP_STYLE} formatter={(v: number) => [`${v}%`]} />
                  <Legend wrapperStyle={{ color: '#9ca3af', fontSize: 11 }} />
                  <Line type="monotone" dataKey="roe" stroke="#22c55e" strokeWidth={2} dot={{ r: 3 }} name="ROE" />
                  <Line type="monotone" dataKey="roa" stroke="#8b5cf6" strokeWidth={2} dot={{ r: 3 }} name="ROA" />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </div>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-800">
            ...
            <p className="text-gray-500 text-xs mt-0.5">All financial figures in $M</p>
          </div>
              ...
              <tbody>
                {[...quarters].reverse().map((q, i) => (
                  <tr key={q.id} className={`border-b border-gray-800/50 ${i % 2 === 0 ? 'bg-gray-800/20' : ''}`}>
                    ...
                    <td className="px-4 py-3 text-purple-400">{usd(q.gross_profit)}</td>
                    ...
                    <td className="px-4 py-3 text-red-400">{usd(q.total_debt)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
