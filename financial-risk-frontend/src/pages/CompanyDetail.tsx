...
import {
  ...
  Activity, ArrowLeft, RefreshCw, Database,
  AlertTriangle, Building2,
} from 'lucide-react';
import Layout from '../components/Layout.tsx';
import api from '../api/axios.ts';

interface Driver { feature: string; importance: number; }
interface Quarter {
  id: number;
  quarter_date: string;
  revenue: number | null;
...
interface Company {
  ...
  sector: string; industry: string; country: string;
  employees: number; market_cap: number; website: string;
  description: string; fetched_at: string;
}

const n   = (v: unknown) => parseFloat(String(v ?? 0)) || 0;
const fmt = (d: string)  => d?.split('T')[0]?.slice(0, 7) ?? d?.slice(0, 7) ?? '';
...
const riskInfo = (label: string) => {
  switch (label) {
    case 'high_risk':   return { text: 'High Risk',   color: '#ef4444', bg: '#ef444415' };
    case 'medium_risk': return { text: 'Medium Risk', color: '#f59e0b', bg: '#f59e0b15' };
    default:            return { text: 'Low Risk',    color: '#22c55e', bg: '#22c55e15' };
  }
};
...
export default function CompanyDetail() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (err.response?.status === 404) {
        setError(`${ticker} has not been analyzed yet. Go to Analyze Company to run a full analysis.`);
      } else {
  ...
  const revChange = latest?.revenue && previous?.revenue
    ? ((n(latest.revenue) - n(previous.revenue)) / n(previous.revenue) * 100).toFixed(1)
    : null;
  ...
  const probData = [
    { name: 'Low',    value: +(n(latest?.prob_low_risk) * 100).toFixed(1),    color: '#22c55e' },
    { name: 'Medium', value: +(n(latest?.prob_medium_risk) * 100).toFixed(1), color: '#f59e0b' },
    { name: 'High',   value: +(n(latest?.prob_high_risk) * 100).toFixed(1),   color: '#ef4444' },
  ];
  ...
  const radarData = [
    ...
    { metric: 'Profitability',value: clamp(n(latest?.operating_margin), -0.1, 0.3) },
    { metric: 'Growth',       value: clamp(n(latest?.revenue_growth_yoy), -0.2, 0.3) },
        ...
        <div className="flex items-start justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <div
              ...
              style={{ color: ri.color, borderColor: ri.color + '40', background: ri.bg }}
            >
              <Shield className="w-4 h-4" />
              <div>
                <p className="font-bold text-sm">{ri.text}</p>
                <p className="text-xs opacity-70">{(n(latest?.confidence) * 100).toFixed(1)}% confidence</p>
              </div>
            </div>
          ...
          <div className="space-y-6">
              ...
              {[
                {
                  ...
                  sub:   revChange ? `${revChange}% vs prev quarter` : 'Latest quarter',
                  icon:  DollarSign,
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
                <h3 className="text-white font-semibold mb-1">Revenue & Net Income</h3>
                ...
                <ResponsiveContainer width="100%" height={200}>
                  <AreaChart data={incomeData}>
                    ...
                    <Area type="monotone" dataKey="revenue" stroke="#3b82f6" fill="url(#rg2)" strokeWidth={2} name="Revenue" />
                    <Area type="monotone" dataKey="net"     stroke="#22c55e" fill="url(#ng2)" strokeWidth={2} name="Net Income" />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
                  ...
                  {[...quarters].reverse().map((q, i) => {
                    ...
                    return (
                      <div key={i} className="flex items-center justify-between py-2 border-b border-gray-800 last:border-0">
                        <div className="flex items-center gap-3">
                          <div className="w-2 h-2 rounded-full" style={{ background: qri.color }} />
                          <span className="text-gray-300 text-sm font-mono">{fmt(q.quarter_date)}</span>
                        </div>
                        <div className="flex items-center gap-3">
                          <span className="text-xs font-medium px-2 py-0.5 rounded-full" style={{ color: qri.color, background: qri.bg }}>
                            {qri.text}
                          </span>
                          <span className="text-gray-500 text-xs">{(n(q.confidence) * 100).toFixed(1)}%</span>
                        </div>
                      </div>
                    );
                  })}
            ...
            {company.description && (
              <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
                <h3 className="text-white font-semibold mb-3 flex items-center gap-2">
                  <Database className="w-4 h-4 text-gray-400" /> About {company.name}
                </h3>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {[
                ...
                { label: 'Operating Margin', value: pct(latest?.operating_margin) },
            ...
            {/* Charts */}
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
              <h3 className="text-white font-semibold mb-1">Income Statement</h3>
              <p className="text-gray-500 text-xs mb-4">Revenue, operating income, net income — $M</p>
              <ResponsiveContainer width="100%" height={240}>
                <AreaChart data={incomeData}>
                  <defs>
                    {[['rg3','#3b82f6'],['og3','#f59e0b'],['ng3','#22c55e']].map(([id,color]) => (
                  ...
                  <Tooltip {...TOOLTIP_STYLE} formatter={(v: number) => [`$${v}M`]} />
                  <Area type="monotone" dataKey="revenue"   stroke="#3b82f6" fill="url(#rg3)" strokeWidth={2} name="Revenue" />
                  <Area type="monotone" dataKey="operating" stroke="#f59e0b" fill="url(#og3)" strokeWidth={2} name="Operating Income" />
                  <Area type="monotone" dataKey="net"       stroke="#22c55e" fill="url(#ng3)" strokeWidth={2} name="Net Income" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
                <h3 className="text-white font-semibold mb-1">Profit Margins</h3>
                ...
                <ResponsiveContainer width="100%" height={200}>
                  <LineChart data={marginData}>
                    ...
                    <Line type="monotone" dataKey="gross"     stroke="#8b5cf6" strokeWidth={2} dot={{ r: 3 }} name="Gross" />
                    <Line type="monotone" dataKey="operating" stroke="#f59e0b" strokeWidth={2} dot={{ r: 3 }} name="Operating" />
                    <Line type="monotone" dataKey="net"       stroke="#22c55e" strokeWidth={2} dot={{ r: 3 }} name="Net" />
                  </LineChart>
                </ResponsiveContainer>
              </div>

              <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
                <h3 className="text-white font-semibold mb-1">Cash Flow & Debt</h3>
                <p className="text-gray-500 text-xs mb-4">Quarterly — $M</p>
                <ResponsiveContainer width="100%" height={200}>
                  <BarChart data={cashData}>
                    ...
                    <YAxis tick={{ fill: '#6b7280', fontSize: 11 }} />
                    <Tooltip {...TOOLTIP_STYLE} formatter={(v: number) => [`$${v}M`]} />
                    <Bar dataKey="fcf"  fill="#22c55e" radius={[4,4,0,0]} name="FCF" />
                    <Bar dataKey="cash" fill="#3b82f6" radius={[4,4,0,0]} name="Cash" />
                    <Bar dataKey="debt" fill="#ef4444" radius={[4,4,0,0]} name="Debt" />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-800">
                      {['Quarter','Revenue','Op. Income','Net Income','FCF','Current Ratio','D/E','Risk'].map(h => (
                  ...
                  <tbody>
                    {[...quarters].reverse().map((q, i) => {
                      const qri = riskInfo(q.risk_label);
                      return (
                        <tr key={q.id} className={`border-b border-gray-800/50 ${i % 2 === 0 ? 'bg-gray-800/20' : ''}`}>
                          <td className="px-4 py-3 text-gray-300 font-mono text-xs">{fmt(q.quarter_date)}</td>
                          <td className="px-4 py-3 text-white">{usd(q.revenue)}</td>
                          <td className="px-4 py-3 text-yellow-400">{usd(q.operating_income)}</td>
                          <td className="px-4 py-3 text-green-400">{usd(q.net_income)}</td>
                          <td className="px-4 py-3 text-blue-400">{usd(q.free_cash_flow)}</td>
                          <td className="px-4 py-3">
                            <span className={n(q.current_ratio) >= 1.5 ? 'text-green-400' : 'text-red-400'}>
                              {n(q.current_ratio).toFixed(2)}
                            </span>
                          </td>
                          <td className="px-4 py-3">
                            <span className={n(q.debt_to_equity) <= 1 ? 'text-green-400' : 'text-red-400'}>
          ...
          <div className="space-y-6">
            ...
            >
              ...
              <div className="flex-1">
                ...
                <p className="text-gray-400 text-sm">Latest: {fmt(latest?.quarter_date ?? '')} · {(n(latest?.confidence) * 100).toFixed(1)}% confidence</p>
              </div>
              <div className="flex gap-6 text-right">
                {[
                  { label: 'Low', value: n(latest?.prob_low_risk), color: '#22c55e' },
                  { label: 'Medium', value: n(latest?.prob_medium_risk), color: '#f59e0b' },
                  { label: 'High', value: n(latest?.prob_high_risk), color: '#ef4444' },
                ].map(p => (
                  <div key={p.label}>
                    <p className="text-gray-400 text-xs">{p.label}</p>
                    <p className="font-bold" style={{ color: p.color }}>{(p.value * 100).toFixed(1)}%</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {/* Probability bars */}
              <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
                <h3 className="text-white font-semibold mb-1">Risk Probabilities</h3>
                <p className="text-gray-500 text-xs mb-4">Latest quarter</p>
                <ResponsiveContainer width="100%" height={160}>
                  <BarChart data={probData} layout="vertical">
                    <CartesianGrid strokeDasharray="3 3" stroke="#1f2937" />
                    <XAxis type="number" tick={{ fill: '#6b7280', fontSize: 11 }} unit="%" domain={[0, 100]} />
              ...
              {/* Radar */}
              <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
                <h3 className="text-white font-semibold mb-1">Financial Health Radar</h3>
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
              ...
              <p className="text-gray-500 text-xs mb-5">Latest quarter — SHAP importance</p>
              <div className="space-y-4">
                {(latest?.top_risk_drivers ?? []).map((d, i) => (
                  <div key={i}>
                    <div className="flex justify-between text-sm mb-1.5">
                      <span className="text-gray-300 capitalize">{d.feature.replace(/_/g, ' ')}</span>
                      <span className="text-gray-400 font-mono">{d.importance.toFixed(4)}</span>
                    </div>
                    <div className="w-full bg-gray-800 rounded-full h-2">
                      <div
                        ...
                        style={{
                          ...
                          background: i === 0 ? '#3b82f6' : i === 1 ? '#8b5cf6' : '#f59e0b',
                        }}
                      />
                    </div>
                  </div>
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-800">
                      {['Quarter','Risk Label','Confidence','Current Ratio','D/E Ratio','Op. Margin','FCF'].map(h => (
                        <th key={h} className="text-left px-4 py-3 text-gray-400 font-medium text-xs">{h}</th>
                      ))}
                    </tr>
                  </thead>
                    ...
                    {[...quarters].reverse().map((q, i) => {
                      ...
                      return (
                        <tr key={q.id} className={`border-b border-gray-800/50 ${i % 2 === 0 ? 'bg-gray-800/20' : ''}`}>
                          ...
                          <td className="px-4 py-3 text-white">{(n(q.confidence) * 100).toFixed(1)}%</td>
                          <td className="px-4 py-3">
                            <span className={n(q.current_ratio) >= 1.5 ? 'text-green-400' : 'text-red-400'}>
                              {n(q.current_ratio).toFixed(2)}
                            </span>
                          ...
                          <td className="px-4 py-3">
                            <span className={n(q.operating_margin) > 0 ? 'text-green-400' : 'text-red-400'}>
                              {(n(q.operating_margin) * 100).toFixed(1)}%
                            </span>
                          </td>
                          <td className="px-4 py-3">
                            <span className={n(q.free_cash_flow) >= 0 ? 'text-green-400' : 'text-red-400'}>
